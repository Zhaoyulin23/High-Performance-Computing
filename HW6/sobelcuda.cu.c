...
__global__ void sobel(unsigned int* pic ,int* result,int width,int height,int thresh){
	...
	__shared__ int pic_s[blockX*blockY];
	
	
	unsigned int index = y*width+x;
	unsigned int sharedIndex = threadIdx.y * blockDim.y + threadIdx.x;
	
	pic_s[sharedIndex] = pic[index];
	
	__syncthreads();
	
	if(		threadIdx.x >= 1 && threadIdx.x < (blockX-1)
			&&	threadIdx.y >= 1 && threadIdx.y <(blockY-1))
	{
		int sum1=0;
		int sum2=0;
		int magnitude=0;
		
		sum1= pic_s[blockDim.y * (threadIdx.y-1)+threadIdx.x+1] - pic_s[blockDim.y * (threadIdx.y-1)+threadIdx.x-1] + 2 * pic_s[blockDim.y * threadIdx.y+threadIdx.x+1] -2 * pic_s[blockDim.y * threadIdx.y+threadIdx.x-1] + pic_s[blockDim.y * (threadIdx.y+1)+threadIdx.x+1] - pic_s[blockDim.y * (threadIdx.y+1)+threadIdx.x-1];
		
		sum2= pic_s[blockDim.y * (threadIdx.y-1)+threadIdx.x-1] + 2 * pic_s[blockDim.y * (threadIdx.y-1)+threadIdx.x] + pic_s[blockDim.y * (threadIdx.y-1)+threadIdx.x+1] - pic_s[blockDim.y * (threadIdx.y+1)+threadIdx.x-1] - 2 * pic_s[blockDim.y * (threadIdx.y+1)+threadIdx.x] - pic_s[blockDim.y * (threadIdx.y+1)+threadIdx.x+1];
...
int main( int argc, char **argv )
{
  ...
  cudaMalloc((void**)&result_d,xsize*ysize*3*sizeof(int));
  
  cudaMemcpy(pic_d,pic,xsize*ysize*sizeof(unsigned int),cudaMemcpyHostToDevice);

//setup block and grid size
  dim3 dimBlock(blockX,blockY);
  dim3 dimGrid((xsize+dimBlock.x-1-2)/(dimBlock.x-2),(ysize+dimBlock.y-1)/(dimBlock.y-2));
...
//free device memory
  ...
  cout<<"Start Time = "<<start<<" Stop Time = "<<stop<<endl;
