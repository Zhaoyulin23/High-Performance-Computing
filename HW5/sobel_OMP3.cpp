...
#pragma omp parallel shared(pic,result,nthreads, y_block) private(sum1, sum2, magnitude, tid) num_threads(nthreads)
{
int offset;
...
#pragma omp parallel for
  ...
  for (i = (y_block*tid)+1; i < (y_block)*(tid+1); i++) {
    for (j = 1; j < xsize -1; j++) {
