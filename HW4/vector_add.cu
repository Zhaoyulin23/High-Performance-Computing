#include<iostream>
#include<cuda.h>
#include "../util/util.h" //to check that output is correct
using namespace std;
const int N = 16;
...
int main(int argc,char ** argv){
	...
	if(vect_len >100){
		cout << "this program has very naive thread layout, " << "please use vector length of less than 100" << endl;
	}
	...
	//function correct is in util.h
	bool correct = verify(vect_len,vect1,vect2,result);
	if(correct){
		cout << "result is correct" << endl;
	}else{
		cout << "result is _NOT_correct" << endl;
	} 
